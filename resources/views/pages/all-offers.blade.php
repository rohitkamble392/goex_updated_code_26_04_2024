                ...
                <div class="col-lg-4">
                    <nav class="breadcrumb-container" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <a href="{{url('add-offers')}}" class="btn btn-secondary text-white">{{ __('Add Offer')}}</a>
                        </ol>
                    </nav>
                </div>
            </div>
                        ...
                        <table id="data_table" class="table table-striped table-hover text-center">
                            <thead class="bg-secondary">
                                <tr>
                                    ...
                                    <th class="text-white"><b>{{ __('Title')}}</b></th>
                                    <th class="text-white"><b>{{ __('Description')}}</b></th>
                                    <th class="text-white"><b>{{ __('Imagefile')}}</b></th>
                                    <th class="text-white"><b>{{ __('Active')}}</b></th>
                                    <th class="nosort text-white"><b>{{ __('Action')}}</b></th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($offerDetails['Result'] as $offer)
                                <tr>
                                    <td class="text-center"><b>{{ $offer['ID'] }}</b></td>
                                    <td class="text-center">{{ $offer['Title'] }}</td>
                                    <td class="text-center">{{ $offer['Description'] }}</td>
                                    <td class="text-center">{{ $offer['Description'] }}</td>
                                    <td class="text-center">
                                        @if ($offer['Active'] == 1)
                                        <span class="bg-secondary text-white px-3 py-1 rounded">Active</span>
                                        @endif
                                    </td>
